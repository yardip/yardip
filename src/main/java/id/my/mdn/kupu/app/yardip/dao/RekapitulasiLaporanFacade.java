...
public class RekapitulasiLaporanFacade extends AbstractSqlFacade<RekapitulasiLaporan> {
    ...
    SELECT ORG0.NAME, RANG4.SISA_AWAL, RANG4.PENERIMAAN_PREV, RANG4.PENERIMAAN_CURR, RANG4.PENGELUARAN_PREV, RANG4.PENGELUARAN_CURR
    FROM (
        SELECT ROL0.PARTY_ID,
               (CASE WHEN RANG3.SISA_AWAL IS NULL THEN 0 ELSE RANG3.SISA_AWAL END) AS SISA_AWAL,
               RANG3.PENERIMAAN_PREV, RANG3.PENERIMAAN_CURR,
               RANG3.PENGELUARAN_PREV, RANG3.PENGELUARAN_CURR
        FROM (
                SELECT BIZ0.ID AS ENTITY_ID, RANG2.SISA_AWAL, RANG2.PENERIMAAN_PREV, RANG2.PENERIMAAN_CURR, RANG2.PENGELUARAN_PREV, RANG2.PENGELUARAN_CURR
                FROM PARTY_BUSINESSENTITY AS BIZ0
                LEFT JOIN (
                ...
                ON BIZ0.ID = RANG2.ENTITY_ID
        ) AS RANG3
        JOIN PARTY_PARTYROLE AS ROL0
        ON RANG3.ENTITY_ID = ROL0.ID
    ) AS RANG4
    JOIN PARTY_ORGANIZATION AS ORG0
    ON RANG4.PARTY_ID = ORG0.ID
    ORDER BY ORG0.ID
    """;
    ...
    protected void setParameters(Query q, Map<String, Object> parameters) {
        ...
        String strPrevThruDate = fmt.format(previousPeriod.getThruDate());

        q.setParameter(3, strPrevThruDate);
